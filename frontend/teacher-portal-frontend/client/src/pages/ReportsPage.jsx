  ...
  return (
    <div className="page-stack">
      <PageHeader
        ...
        action={<span className="page-icon"><BarChart3 size={20} /></span>}
      />
          ...
          {reports.loading ? (
            ...
            <div className="report-items">
              {list.map((report, index) => (
                <button
                  className={`report-item ${selected?.id === report?.id || selected?._id === report?._id ? 'selected' : ''}`}
                  key={report?.id || report?._id || index}
                  onClick={() => select(report)}
                >
                  <span className="report-index">{String(index + 1).padStart(2, '0')}</span>
                  <span>
                    <strong>{displayValue(report?.title, 'Untitled report')}</strong>
                    <small>{displayValue(report?.createdAt || report?.date, 'Date unavailable')}</small>
                  </span>
                </button>
              ))}
            </div>
