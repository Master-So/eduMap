...
export default function StudentsPage() {
  ...
  const disconnectStudent = async (studentId) => {
    try {
      await studentService.disconnectStudent(studentId);
      await students.retry();
    } catch (error) {
      window.alert(error?.message || 'Unable to disconnect student.');
    }
  };
  return <div className="page-stack">
    ...
    <section className="panel table-panel"><div className="panel-heading"><div><span className="eyebrow">Connected students</span><h2>Your student roster</h2></div></div>{students.loading ? <LoadingState label="Loading students..." /> : students.error ? <ErrorState message={students.error} onRetry={students.retry} /> : <StudentTable students={studentList} onDisconnect={disconnectStudent} />}</section>
  </div>;
}
